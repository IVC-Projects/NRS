...
from TRAIN_CNNX import BATCH_SIZE
from TRAIN_CNNX import PATCH_SIZE
import struct
import re
...
def get_train_list2(lowList, highList):
    assert len(lowList) % len(highList)==0, "low:%d, high:%d"%(len(lowList), len(highList))
    train_list = []
...
def get_train_list3(lowList, highList):
    assert len(lowList) % len(highList)==0, "low:%d, high:%d"%(len(lowList), len(highList))
    train_list = []
    for i in range(300,1000):
        # print(lowList[i])
        qp=lowList[i].split("\\")[-2].split("q")[1]
        if 37<=int(qp)<=46:
            train_list.append([lowList[i], highList[i%len(highList)]])
    return train_list

def get_train_list4(lowList, highList):
    assert len(lowList) % len(highList)==0, "low:%d, high:%d"%(len(lowList), len(highList))
    train_list = []
...
def getWH(yuvfileName):   # Train
    ...
    # print(os.path.basename(deyuvFilename))
