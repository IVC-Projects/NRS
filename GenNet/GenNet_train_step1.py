...
# from skimage.measure import compare_ssim as ssim tensorflow2.4不支持
...
tf.app.flags.DEFINE_string('train_dir', r'',
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.app.flags.DEFINE_string('train_image_dir', './train_output_image/',
                           """Directory where to output images.""")
tf.app.flags.DEFINE_string('test_image_dir', './test_output_image_baseline/',
                           """Directory where to output images.""")
...
def train(dataset_frame1, dataset_frame2, dataset_frame3):
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("Cycle_DVF"):
            ...
            print("***********************************************************************")
            print(input1.shape,input3.shape,edge_1.shape,edge_3.shape)
            print(tf.concat([input1, input3, edge_1, edge_3], 3).shape)
            print("***********************************************************************")
...
def test(dataset_frame1, dataset_frame2, dataset_frame3):
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("Cycle_DVF"):
            # Prepare model.
            model = Voxel_flow_model(is_train=False,is_extrapolation=True)
        ...
        #         print(os.path.join('ucf101_interp_ours', ll)[:-5] + '00.png')

        for id_img in range(0, data_size):
            UCF_index = data_list_frame1[id_img][:-12]
            # Load single data.

            batch_data_frame1 = [dataset_frame1.process_func(os.path.join('ucf101_interp_ours', ll)[:-5] + '0.png') for
                                 ll in data_list_frame1[id_img:id_img + 1]]
            batch_data_frame2 = [dataset_frame2.process_func(os.path.join('ucf101_interp_ours', ll)[:-5] + '1.png')
                                 for ll in data_list_frame2[id_img:id_img + 1]]
            batch_data_frame3 = [dataset_frame3.process_func(os.path.join('ucf101_interp_ours', ll)[:-5] + '2.png') for
